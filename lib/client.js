...
class SonnyLabsClient {
    ...
    try {
      ...
      if (response.status === 200) {
        if (!response.data.analysis && response.data.message) {
          ...
          if (
            response.data.message.includes("200 OK") ||
            response.data.message.includes("confirmation_sent_at")
          ) {
   ...
   * @param {string} text
   * @param {string} [scanType='input']
   * @param {object} [policy]
   * @param {object} [meta]
   * @returns {Promise<ScanVerdict>}
   */
  async scanText(text, scanType = 'input', policy = {}, meta = {}) {
    return helper.scanText(text, this, scanType, policy, meta);
  }
   ...
   * @returns {Promise<ScanVerdict>}
   */
  async scanMessages(messages, scanType = 'input', policy = {}, meta = {}) {
    return helper.scanMessages(messages, this, scanType, policy, meta);
  }
   ...
   * @param {object} [policy]
   * @param {object} [meta]
   * @returns {Promise<RagScanResult>}
   */
  async scanRagChunks(query, chunks, policy = {}, meta = {}) {
    return helper.scanRagChunks(query, chunks, this, policy, meta);
  }
   ...
   * @param {object|string} [toolSchema]
   * @param {object} [policy]
   * @param {object} [meta]
   * @returns {Promise<ToolCallScanResult>}
   */
  async scanToolCall(userMessage, toolName, toolArgs, toolSchema = null, policy = {}, meta = {}) {
    return helper.scanToolCall(userMessage, toolName, toolArgs, this, toolSchema, policy, meta);
  }
}
